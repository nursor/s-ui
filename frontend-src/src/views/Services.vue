  ...
  <v-row>
    <v-col cols="12" sm="4" md="3" lg="2" v-for="(item, index) in <any[]>services" :key="item.tag">
      ...
      <FRPCard
        ...
        :service="item"
        @edit="showModal"
        @delete="handleFRPDelete"
      />
      <!-- 其他服务使用通用卡片 -->
      <v-card v-else rounded="xl" elevation="5" min-width="200" :title="item.tag">
...
import FRPCard from '@/components/services/FRPCard.vue'
...
const handleFRPDelete = (id: number) => {
  const index = services.value.findIndex(i => i.id == id)
  if (index !== -1) {
    delOverlay.value[index] = true
  }
}
