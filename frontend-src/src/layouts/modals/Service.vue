  ...
  <v-dialog transition="dialog-bottom-transition" width="800">
    <v-card class="rounded-lg">
      ...
      <v-card-text style="padding: 0 16px; overflow-y: scroll;">
        ...
        <Listen :data="srv" :inTags="inTags" v-if="srv.type !== srvTypes.FRP" />
        ...
        <FRP v-if="srv.type == srvTypes.FRP" :data="srv" />
...
import FRP from '@/components/services/FRP.vue'
...
export default {
  ...
  methods: {
    ...
    changeType() {
      ...
      // Include listen fields only for non-FRP services
      // Use optional chaining to safely access fields that may not exist on FRP
      if (this.srv.type !== SrvTypes.FRP) {
        prevConfig.listen = (this.srv as any).listen || '::'
        prevConfig.listen_port = (this.srv as any).listen_port || RandomUtil.randomIntRange(10000, 60000)
      }
  ...
  components: { Listen, InTLS, Derp, SSMapi, FRP },
}
</script>